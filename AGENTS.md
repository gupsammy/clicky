...
macOS menu bar companion app. Lives entirely in the macOS status bar (no dock icon, no main window). Clicking the menu bar icon opens a custom floating panel with companion voice controls. `control + fn` performs literal focused-field dictation. When a readable editable field is focused, `control + option` uses the field, its screen, and Codex vision to compose text and inserts one atomic result; otherwise the same shortcut retains the spoken screen-aware companion. Voice transcription uses OpenAI Realtime when configured and Apple Speech without paid proxy configuration. Subscription-backed Codex app-server produces companion and composition responses; the system voice speaks them. A blue cursor overlay can fly to and point at UI elements the model references on any connected monitor.

No API key is required for the primary ChatGPT-subscription path. Optional legacy provider keys live on a Cloudflare Worker proxy, authenticated with a per-install token stored in the macOS Keychain; nothing sensitive ships in the app bundle.
...
- **AI Chat**: Persistent response-only Codex app-server threads use the user's ChatGPT subscription, server-selected default model, read-only sandbox, local screen images, and disabled MCP servers. Claude through the Worker remains an optional configured fallback.
...
- **Text-to-Speech**: macOS system speech by default; ElevenLabs (`eleven_flash_v2_5`) remains an optional configured Worker voice
- **Screen Capture**: ScreenCaptureKit (macOS 14.2+), multi-monitor support
- **Screen-Aware Composition**: A response-only Codex app-server thread receives one bounded focused-field context plus a local image of only the display containing that field. The app atomically inserts the returned text only if the original value and selection are unchanged. OpenAI Responses through the Worker remains optional fallback.
...
- **Element Pointing**: The companion model embeds `[POINT:x,y:label:screenN]` tags in responses. The overlay parses these, maps coordinates to the correct monitor, and animates the blue cursor along a bezier arc to the target.
- **Spatial Interaction**: While the companion shortcut is held, Clicky records a bounded display-aware cursor trace and can use it as attention context for the captured screen. Model-authored `[TARGET:...]` and `[HOVER:...]` tags render one passive click-through walkthrough step at a time; a matching click or 650 ms dwell recaptures fresh screens and continues the same companion thread.
...
### Optional Legacy API Proxy (Cloudflare Worker)

Legacy paid-provider requests go through a Cloudflare Worker (`worker/src/index.ts`) that holds the real API keys as secrets. The primary Codex app-server companion, composition, and agent paths do not use this proxy.
...
| `POST /openai-screen-compose` | `api.openai.com/v1/responses` | Produces bounded screen-aware focused-field insertion text. |
...
Worker vars: `ELEVENLABS_VOICE_ID`, `VERTEX_PROJECT_ID`, `VERTEX_REGION`, `OPENAI_SCREEN_COMPOSITION_MODEL` (optional; defaults to `gpt-5.6-luna`).
...
**Worker Authorization and Rate Limits**: Every paid proxy route requires a deployment-specific bearer token. Keep it in a Worker secret and the macOS Keychain; never put it in Info.plist or source. Cloudflare rate-limit bindings cap general API calls, Realtime-token minting, and high-detail screen composition separately. Production multi-user distribution must replace the single self-host token with short-lived per-user or per-install sessions issued by an authenticated backend.
...
**Screen-Aware Dictation**: `control + option` becomes a slower composition lane when a readable editable field is focused. Clicky samples bounded surrounding Accessibility text, the selection, app/window identity, and the one display containing the focused element. A dedicated response-only Codex thread receives the text plus a temporary local screenshot path under read-only/no-tool instructions. The app deletes the attachment after the response and revalidates the original app, exact element, whole value, and selection before atomic insertion. Field or focus changes fail closed; no clipboard is used. A configured Worker/OpenAI Responses route remains fallback. If the field cannot safely expose context, the shortcut preserves the conversational companion path.
...
**Response-Only Codex Threads**: Companion vision and screen-aware composition use a separate lightweight app-server process launched with MCP servers disabled. It selects the server-advertised default model, keeps one ephemeral in-session companion thread for conversation and prompt-cache continuity, and starts a fresh ephemeral composition thread so unrelated field text never shares context. Turns send heterogeneous text/local-image inputs, deny approvals, use read-only sandboxing, reject server tool requests, and capture final text from ordered app-server notifications. Cancellation interrupts the active turn. Experimental realtime audio is not the primary path because current app-server versions require API-key auth for that surface.
...
| `CompanionManager.swift` | ~1994 | Central state machine. Owns shortcut routing, literal and screen-aware dictation, focused-field insertion, screen capture, subscription-backed companion turns, optional legacy Worker fallbacks, system/ElevenLabs speech, and overlay management. Tracks voice state, conversation history, cursor visibility, user traces, static annotations, and interactive walkthrough lifetime. |
...
| `CompanionPanelView.swift` | ~758 | SwiftUI panel content for the menu bar dropdown. Shows companion status, push-to-talk and agent-trigger instructions, permissions UI, DM feedback button, and quit button. Dark aesthetic using `DS` design system. |
| `OverlayWindow.swift` | ~959 | Full-screen transparent overlay hosting the blue cursor, response text, waveform, spinner, cursor traces, static spatial annotations, and interactive walkthrough targets. Handles cursor following, element pointing with bezier arcs, multi-monitor coordinate mapping, and fade-out transitions. |
| `CompanionResponseOverlay.swift` | ~217 | SwiftUI view for the response text bubble and waveform displayed next to the cursor in the overlay. |
| `SpatialAnnotationOverlayView.swift` | ~552 | Click-through renderers for cyan user traces, coral TARGET/HOVER steps, and ordered display-bound points, highlights, lines, arrows, circles, curves, polygons, and labels. |
| `CompanionScreenCaptureUtility.swift` | ~344 | ScreenCaptureKit capture. Companion chat can capture all displays; screen-aware dictation resolves and captures only the focused field's display. Writes model attachments to a per-request temporary directory, maps bounded user traces to exact captures, and removes attachments after use. |
...
| `FocusedTextInsertionService.swift` | ~532 | Captures and revalidates editable focused Accessibility elements, rejects secure fields, extracts bounded field/window context, preserves stale-value/selection preconditions, inserts final text through native text attributes, and verifies a clipboard-free Unicode event fallback. |
...
| `DictationCore/ScreenAwareCompositionProtocol.swift` | ~165 | Bounded request/response contract, app-server text prompt shaping, and fail-closed focused-display selection for screen-aware composition. |
| `OpenAIScreenCompositionClient.swift` | ~84 | Sends an authorized bounded screen-aware request to the configured Worker and validates its atomic insertion response. |
| `ClickyProxyAuthorization.swift` | ~64 | Reads the deployment-specific Worker bearer token from the macOS Keychain and authorizes proxy requests without embedding it in the app. |
...
| `DictationCore/SpatialAnnotationGrammar.swift` | ~398 | Foundation-only parser, fail-closed display resolver, and screenshot-to-display coordinate mapper for bounded POINT, HIGHLIGHT, and static SHAPE response tags. |
| `GlobalPushToTalkShortcutMonitor.swift` | ~310 | System-wide push-to-talk monitor. Owns the listen-only `CGEvent` tap, resolves exact fast-dictation versus companion chords, requires a neutral state between chords, captures throttled companion cursor traces, and passively observes active TARGET/HOVER steps. |
| `ClaudeAPI.swift` | ~291 | Claude vision API client with streaming (SSE) and non-streaming modes. TLS warmup optimization, image MIME detection, conversation history support. |
| `OpenAIAPI.swift` | ~142 | OpenAI GPT vision API client. |
| `ElevenLabsTTSClient.swift` | ~81 | ElevenLabs TTS client. Sends text to the Worker proxy, plays back audio via `AVAudioPlayer`. Exposes `isPlaying` for transient cursor scheduling. |
| `ElementLocationDetector.swift` | ~335 | Detects UI element locations in screenshots for cursor pointing. |
| `DesignSystem.swift` | ~880 | Design system tokens — colors, corner radii, shared styles. All UI references `DS.Colors`, `DS.CornerRadius`, etc. |
| `ClickyAnalytics.swift` | ~176 | PostHog analytics integration; dictation events contain only coarse latency, method, and character-count metadata. |
...
| `AgentCore/CodexAgentTaskModels.swift` | ~232 | HUD-independent task, typed compaction/activity, approval, structured user-input, idle, attention-announcement, and event models for concurrent agent progress. |
...
| `AgentCore/CodexAgentCoordinator.swift` | ~296 | Workspace-scoped orchestration for account refresh/login, history, autonomous thread/turn start, same-thread follow-up, steer, interrupt, typed approvals, and structured user-input responses. |
| `AgentCore/CodexCompanionService.swift` | ~492 | Dedicated response-only app-server actor for ephemeral companion/composition threads, server-default model selection, local-image turns, notification-backed response capture, cancellation interrupt, bounded timeout handling, and process-session recovery. |
| `AgentPresentationModel.swift` | ~1022 | Main-actor projection from snapshots, identity-bound authentication lifecycle, and process failures into workspace-filtered routes, forms, approvals/input, persistent attention delivery, spoken start/follow-up dispatch, explicit token dismissal, and low-frequency layout revisions. |
...
| `AgentCoreTests/CodexAgentAttentionTests.swift` | ~100 | Regression tests that keep detailed input/approval content visible while restricting TTS to one generic human-facing attention announcement. |
| `AgentCoreTests/CodexCompanionServiceTests.swift` | ~742 | Response-only service tests for model selection, read-only/no-tool inputs, companion context, isolated composition threads, notification ordering, local images, failure recovery, timeout interrupt, cancellation, and an opt-in live image turn. |
| `DictationCore/SpatialInteractionProtocol.swift` | ~622 | Bounded cursor-trace capture, display-aware screenshot normalization, compact hover summaries, TARGET/HOVER tag parsing, and exactly-once interaction reduction. |
| `DictationCoreTests/SpatialInteractionProtocolTests.swift` | ~387 | Deterministic trace bounds, coordinate normalization, hover summary, tag sanitization, cross-display failure, TARGET click, HOVER dwell, and repeat-protection tests. |
...
| `DictationCoreTests/ScreenAwareCompositionProtocolTests.swift` | ~101 | Request/response bounds, screenshot encoding, and fail-closed multi-display target-selection tests. |
...
| `worker/src/index.ts` | ~515 | Authenticated and rate-limited Cloudflare proxy for chat, TTS, transcription tokens, ephemeral OpenAI Realtime secrets, and completed-only OpenAI Responses screen composition. |
